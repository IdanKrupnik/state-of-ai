          ...
          {metrics.map((m, i) => (
            <div key={i} className="border border-outline-variant/20 p-4 bg-brand-offwhite/50 hover:border-brand-black transition-all">
              <button onClick={() => setOpenIndex(openIndex === i ? null : i)} className="w-full text-left flex justify-between items-start gap-4 cursor-pointer" data-testid={`metric-btn-${i}`}>
                <div>
                  <span className="block text-base font-bold text-brand-black">{m.title}</span>
              ...
              <AnimatePresence>
                {openIndex === i && (
                  <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }} className="overflow-hidden mt-4 pt-4 border-t border-outline-variant/20 flex flex-col gap-3 text-xs text-brand-warm-grey" data-testid={`metric-details-${i}`}>
                    <div>
                      <strong className="block text-red-500 mb-1">🚨 Why this points to a Bubble:</strong>
                      <p>{m.bubble}</p>
                    </div>
                    <div>
                      <strong className="block text-indigo-600 mb-1">✅ Why this points to a Stable Build:</strong>
                      <p>{m.stable}</p>
                    </div>
                  </motion.div>
                )}
